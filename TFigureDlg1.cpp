...
// 		pDC.SetBkMode(TRANSPARENT);
	...
	{	
		...
		for(int i=0;i<8;i++)
		{
			...
			if(i<7)
			{
				dc.TextOutW(m_pPointEightLeft[i].x,m_pPointEightRight[i].y,m_strAVS[i]);
			} 
			left.y+=60;
...
// 	m_strAVS[6]="A";
	...
	for(int i=0;i<8;i++)
	{
		...
		{
			dc.TextOutW(m_pPointEightLeft[i].x,m_pPointEightLeft[i].y,m_strAVS[i]);
		} 
	}
	...
	{	
		...
		for(int i=0;i<4;i++)
		{
			...
			if(i<3)
			{
				dc.TextOutW(m_pPointFourLeft[i].x,m_pPointFourLeft[i].y,m_strAVS[i]);
			} 
			left.y+=60;
...
// 	m_strAVS[2]="V";
	...
	{	
		...
		for(int i=0;i<6;i++)
		{
			...
			{
				dc.TextOutW(m_pPointSixLeft[i].x,m_pPointSixLeft[i].y,m_strAVS[i]);
			}
			left.y+=60;
...
void CTFigureDlg::OnBnClickedOpenfilebutton()
{
	...
	CFileDialog dlg(TRUE,NULL,NULL,
		...
		_T("json数据文件(*.json)|*.json|xml数据文件(*.xml)|*.xml|文本文件(*.txt)|*.txt|所有文件(*.*))|*.*||"),this);
	if(dlg.DoModal() == IDOK)
	{
#if DEBUGS:
...
void CTFigureDlg::ClearPoint()
{
	...
	{
		for (int j=0; j<sizeof(m_arrHeadPoint)/sizeof(CPoint*)-5;j++)
		{
			this->m_arrHeadPoint[j][i].x = 0;
...
void CTFigureDlg::OnBnClickedButton2()  //保存 另存为
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		CString strFileImg  = cstr1 + CString("jpg");
#if DEBUGS:
...
// 	/* MemDC.MoveTo(……); MemDC.LineTo(……);*/
	...
	for (int m= 0;m<(rP.x-lP.x)/20*5-1;m++)
	{
		lPoint2.x += 4;
		for (int n =0;n<(rP.y-lP.y)/20*5-1;n++)
		{
			lPoint2.y += 4;
...
void CTFigureDlg::Draw2Arrow(CPoint p1,CPoint p2,int nSeg,CDC* dc)
{
	...
	{
		a[i].x = a[i-1].x + lengthX/nSeg ;
		a[i].y = a[i-1].y + lengthY/nSeg ;
	}
	//CClientDC dc(this);
	CPen pen(PS_SOLID,2,RGB(0,0,0));
	dc->SelectObject(&pen);
...
{	
	...
	//dc.MoveTo(a[i]);
	//dc.LineTo(a[i+1]);
	DrawArrow(a[0],a[1],90.0,10,dc);
	DrawArrow(a[3],a[2],90.0,10,dc);
	dc->SetROP2(nOldMode);
}
...
void CTFigureDlg::DrawTententenB(CPoint p1,CPoint p2,int nSeg)
{
	...
	CPen pen(PS_SOLID,2,RGB(0,0,0));
	dc.SelectObject(&pen);
	DrawArrow(a[0],a[3],90.0,10);
	DrawArrow(a[3],a[4],90.0,10);
	DrawArrow(a[4],a[5],90.0,10);
	dc.MoveTo(a[5]);
	dc.LineTo(a[8]);
	//DrawPointBetweenTwoPoint(p1,CPoint((p1.x+p2.x)/2,(p1.y+p2.y)/2),90.0,10);
...
void CTFigureDlg::DrawTententenB(CPoint p1,CPoint p2,int nSeg,CDC* dc)
{
	...
	dc->SelectObject(&pen);
	int nOldMode = dc->SetROP2(R2_NOT);
	DrawArrow(a[0],a[3],90.0,10,dc);
	DrawArrow(a[3],a[4],90.0,10,dc);
	DrawArrow(a[4],a[5],90.0,10,dc);
	dc->MoveTo(a[5]);
	dc->LineTo(a[8]);
	dc->SetROP2(nOldMode);
...
// 	CString(m_cXmlFilePath.c_str()).ReleaseBuffer();
	root["XmlFileName"] =Json::Value(m_cXmlFilePath); 
...
void CTFigureDlg:: readfireadFileJson(CString cs)
{
	...
	//cstring转char *
	USES_CONVERSION;
	const char* strbuf = T2A(csstring.GetBuffer(0));
	...
	if (reader.parse(strbuf, root))
	{
		...
		char* cXmlFilename = new char[1000] ;
		strcpy(cXmlFilename,root["XmlFileName"].asString().c_str());
		if(strlen(root["XmlFileName"].asString().c_str())>2)
		{
			m_cXmlFilePath = root["XmlFileName"].asString();
...
//长文本输入
void CTFigureDlg::LongTextInput(CRect rect)
{
	static int i = 0 ;
	CString str(_T("输入文字"));
	CEdit* pEdit  = new CEdit;
	pEdit->Create(WS_CHILD|WS_VISIBLE|WS_TABSTOP |WS_BORDER|ES_LEFT|ES_AUTOHSCROLL|\
		ES_MULTILINE|ES_WANTRETURN|ES_AUTOVSCROLL |WS_HSCROLL| WS_VSCROLL, rect, this, ID_EDIT1+ i);
...
void CTFigureDlg::DrawDistenceText(CPoint* headPoint)
{
	...
	for (int n=0;n<this->i-1;n++)
	{
		if (vecPointTemp[n].x != 0)
		{
				...
				CString strDistance(tem);
				dc.TextOutW( (vecPointTemp[n].x+vecPointTemp[n+1].x)/2 - 20,vecPointTemp[n+1].y - 10,strDistance);
